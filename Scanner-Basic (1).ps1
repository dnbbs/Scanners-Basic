...
Write-Host " https://discord.gg/qdsG44Jz88         " -ForegroundColor Magenta
Write-Host "========================================" -ForegroundColor Magenta
Write-Host ""
...
$services = "SysMain", "PcaSvc", "DPS", "EventLog", "bam", "Appinfo", "windefend"
...
if ($svc) {
    ...
    if ($sysPath -and (Test-Path $sysPath)) {
        ...
        if ($version -match "^15") {
            Write-Host " -> STATUS: UPDATED âœ…" -ForegroundColor Green
        }
        elseif ($version -match "^13|^14") {
            Write-Host " -> STATUS: OK (not latest) âš ï¸" -ForegroundColor Yellow
        }
        else {
            Write-Host " -> STATUS: OUTDATED âŒ" -ForegroundColor Red
        }
...
try {
    ...
    if ($AnyDesk) {
        ...
        foreach ($Evt in $Events) {
            ...
            # Somente eventos anteriores à abertura do AnyDesk
...
Write-Host "║                  EVENT ID 1116                     ║" -ForegroundColor Cyan
Write-Host "╚══════════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    if (-not $Events) {
        ...
        Write-Host "  [✓] Nenhuma detecção encontrada." -ForegroundColor Green
        Write-Host ""
        ...
        foreach ($Event in $Events) {
            ...
            foreach ($Field in @(
                ...
                if ($Data.ContainsKey($Field) -and $Data[$Field]) {
            ...
            if ($FileName) {
                ...
                foreach ($AllowedName in $WhitelistExactNames) {
            ...
            if (-not $IsWhitelisted -and $FilePath -and $FileName) {

                $IsTempPath = $FilePath -match '(?i)(\\|/)Temp(\\|/)'

                $StartsWithTx = $FileName.StartsWith(
                    "tx_",
                    [System.StringComparison]::OrdinalIgnoreCase
                )

                if ($IsTempPath -and $StartsWithTx) {
                    $IsWhitelisted = $true
                }
            }
            ...
            Write-Host "│ [!] DETECÇÃO DO MICROSOFT DEFENDER                  │" -ForegroundColor Red
            Write-Host "└──────────────────────────────────────────────────────┘" -ForegroundColor Red
            Write-Host ""

            Write-Host "  Data       : " -NoNewline
            Write-Host $Event.TimeCreated.ToString(
                "dd/MM/yyyy HH:mm:ss"
            ) -ForegroundColor DarkGray

            Write-Host "  Event ID   : " -NoNewline
            Write-Host "1116" -ForegroundColor Yellow

            if ($Data.ContainsKey("ThreatName") -and $Data["ThreatName"]) {

                Write-Host "  Ameaça     : " -NoNewline
                Write-Host $Data["ThreatName"] -ForegroundColor Yellow
            }

            if ($Data.ContainsKey("SeverityName") -and $Data["SeverityName"]) {

                Write-Host "  Severidade : " -NoNewline
                Write-Host $Data["SeverityName"] -ForegroundColor Magenta
            }
            ...
            if ($Data.ContainsKey("CategoryName") -and $Data["CategoryName"]) {

                Write-Host "  Categoria  : " -NoNewline
                Write-Host $Data["CategoryName"] -ForegroundColor White
            }
        ...
        if ($Count -eq 0) {
            ...
            Write-Host "  [✓] Nenhuma detecção após a whitelist." -ForegroundColor Green

        }
        else {

            Write-Host "  Detecções encontradas : " -NoNewline
    ...
    Write-Host ""
    Write-Host "[!] Erro ao consultar o Event ID 1116." -ForegroundColor Red
...
Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "          PROCESS MODULE SIGNATURE SCANNER" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
...
$DllWhitelist = @(
    ...
    "C:\Users\cliente\AppData\Local\Discord\app-1.0.9251\profapi.dll"
)

$DllWhitelistDirectories = @(
    "C:\Program Files\AMD\CNext\CNext",
    "C:\Program Files\WindowsApps\Microsoft.WidgetsPlatformRuntime_",
    "C:\Program Files\WindowsApps\5319275A.WhatsAppDesktop_"
)

$NativeImagesPath = "C:\Windows\assembly\NativeImages_v4.0.30319_64"

function Test-WhitelistedDirectory {
    ...
    foreach ($Directory in $DllWhitelistDirectories) {
        ...
        if ($NormalizedDirectory.EndsWith("_")) {
...
foreach ($Process in $Processes) {
    ...
    try {
        ...
        foreach ($Module in $Modules) {
            ...
            foreach ($AllowedDll in $DllWhitelist) {
            ...
            if (Test-WhitelistedDirectory -Path $ModulePath) {
                continue
            }
            ...
            if ($Signature.Status -eq "NotSigned") {
                ...
                Write-Host ""
                Write-Host "  Arquivo: $ModulePath" -ForegroundColor Red
                Write-Host "  Status: NotSigned" -ForegroundColor Red
            ...
            if ($Signature.Status -eq "HashMismatch") {
                ...
                Write-Host ""
                Write-Host "  Arquivo: $ModulePath" -ForegroundColor Blue
                Write-Host "  Status: HashMismatch" -ForegroundColor Blue
    ...
    if ($HasInvalidModules) {
        ...
        Write-Host ""
        Write-Host "  [!] Processo potencialmente suspeito:" `
...
Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "             APPLICATION CRASH SCANNER" -ForegroundColor Cyan
Write-Host "                    EVENT ID 1000" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
...
$WhitelistTempPrefixes = @(
    "ox_17",
    "BK"
)

function Test-CrashWhitelist {
    ...
    if ($AppPath) {
        ...
        foreach ($AllowedPath in $WhitelistExactPaths) {
        ...
        $IsTemp = $NormalizedPath -match '(?i)(\\|/)Temp(\\|/)'

        if ($IsTemp -and $FileName) {

            foreach ($Prefix in $WhitelistTempPrefixes) {
    ...
    if ($AppName) {
        ...
        $CleanName = [System.IO.Path]::GetFileName($AppName)
...
try {
    ...
    if (-not $Events) {
        ...
        Write-Host "  [✓] Nenhum aplicativo crashado encontrado." `
        ...
        foreach ($Event in $Events) {
            ...
            if ($Data.ContainsKey("AppPath")) {
                $AppPath = $Data["AppPath"]
            }

            if (Test-CrashWhitelist `
                -AppName $AppName `
                -AppPath $AppPath) {
            ...
            Write-Host "  Data/Hora     : " -NoNewline
            Write-Host $Event.TimeCreated.ToString(
                "dd/MM/yyyy HH:mm:ss"
            ) -ForegroundColor DarkGray

            Write-Host "  Event ID      : " -NoNewline
            Write-Host "1000" -ForegroundColor Yellow

            if ($AppName) {

                Write-Host "  Aplicativo    : " -NoNewline
                Write-Host $AppName -ForegroundColor White
            }

            if ($Data.ContainsKey("AppVersion") -and
                $Data["AppVersion"]) {

                Write-Host "  Versão App    : " -NoNewline
                Write-Host $Data["AppVersion"] -ForegroundColor Gray
            }

            if ($AppPath) {

                Write-Host "  Caminho App   : " -NoNewline
                Write-Host $AppPath -ForegroundColor White
            }

            if ($Data.ContainsKey("ModuleName") -and
                $Data["ModuleName"]) {

                Write-Host "  Módulo        : " -NoNewline
                Write-Host $Data["ModuleName"] -ForegroundColor Yellow
            }

            if ($Data.ContainsKey("ModuleVersion") -and
                $Data["ModuleVersion"]) {

                Write-Host "  Versão Módulo : " -NoNewline
                Write-Host $Data["ModuleVersion"] -ForegroundColor Gray
            }

            if ($Data.ContainsKey("ModulePath") -and
                $Data["ModulePath"]) {

                Write-Host "  Caminho Módulo: " -NoNewline
                Write-Host $Data["ModulePath"] -ForegroundColor White
            }

            if ($Data.ContainsKey("ExceptionCode") -and
                $Data["ExceptionCode"]) {

                Write-Host "  Exception     : " -NoNewline
                Write-Host $Data["ExceptionCode"] -ForegroundColor Magenta
            }

            if ($Data.ContainsKey("FaultingOffset") -and
                $Data["FaultingOffset"]) {

                Write-Host "  Fault Offset  : " -NoNewline
                Write-Host $Data["FaultingOffset"] -ForegroundColor DarkYellow
            }
        ...
        if ($Count -eq 0) {
            ...
            Write-Host "  [✓] Nenhum crash após a whitelist." `
                -ForegroundColor Green

        }
        else {

            Write-Host "  Crashes encontrados : " -NoNewline
            Write-Host $Count -ForegroundColor Red
        }
    ...
    Write-Host ""
    Write-Host "  [!] Erro ao consultar o Event ID 1000." `
        -ForegroundColor Red
