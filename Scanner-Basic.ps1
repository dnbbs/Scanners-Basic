...
Write-Host "`n[*] KEYWORDS SCANNER" -ForegroundColor Cyan

$keywords = @(
    "hd-player","chams","aimbot","aim silent","aim legit"
)

$extensions = @("*.exe","*.dll","*.tmp","*.dat")
...
foreach ($ext in $extensions) {
    ...
    Get-ChildItem C:\ -Recurse -Filter $ext -File -ErrorAction SilentlyContinue |
    ...
    ForEach-Object {
        ...
        try {
            ...
            $counter++
            Write-Progress -Activity "Scanning C:\" -Status "$counter arquivos" -PercentComplete ($counter % 100)
            ...
            if ($found.Count -gt 0) {
                ...
                Write-Host "`n[!!!] MATCH" -ForegroundColor Red
                Write-Host "File: $($file.FullName)" -ForegroundColor Yellow
                Write-Host "Keywords: $($found -join ', ')" -ForegroundColor Cyan
            }
...
Write-Progress -Activity "Scanning C:\" -Completed
