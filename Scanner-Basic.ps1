...
Write-Host "[*] KEYWORDS SCANNER" -ForegroundColor Cyan

$keywords = @(
    "hd-player","chams","aimbot","aim silent","aim legit", "gldraw", "glesp", "glaim", "glaimbot", "circle fov", "child5", "dll free fire", "glDrawElements", "glDrawPixels", "glDrawHealth"
)

$extensions = @(".dll",".dat",".exe",".tmp", ".bak")
...
try {
    ...
    if ($shadowCopies) {
        foreach ($shadow in $shadowCopies) {
            if ($shadow.DeviceObject) {
                $paths += "$($shadow.DeviceObject)\"
            }
        }
        Write-Host "[VSS] Loaded: $($shadowCopies.Count) shadow copies" -ForegroundColor Green
    }
} catch {}
...
foreach ($path in $paths) {
    ...
    Write-Host "`n[SCAN] $path" -ForegroundColor DarkCyan
    ...
    ForEach-Object {
        ...
        try {
            ...
            foreach ($i in $ignore) {
                if ($lower.StartsWith($i)) { return }
            }

            if ($file.Length -gt 50MB -or $file.Length -lt 5KB) { return }

            $counter++

            if ($counter % 50 -eq 0) {
                Write-Progress -Activity "Scanning..." -Status "$counter files scanned"
            }
            ...
            foreach ($word in $keywords) {
                if ($ascii.ToLower().Contains($word) -or $unicode.ToLower().Contains($word)) {
                    $found += $word
                }
            }
            ...
            if ($found.Count -gt 0) {
                ...
                Write-Host "Keywords: $($unique -join ', ')" -ForegroundColor Cyan
            }
...
$HD = Get-Process HD-Player -ErrorAction SilentlyContinue | Select-Object -First 1

if (!$HD)
{
    Write-Host ""
    Write-Host "[ERRO] HD-Player nao encontrado." -ForegroundColor Red
    Write-Host ""
...
Write-Host " HD-PLAYER DETECTADO"
...
Get-WinEvent -FilterHashtable @{
    ...
    $_.Message -match "Image:.*HD-Player.exe"

} | ForEach-Object {

    $msg = $_.Message

    $dll = ($msg | Select-String "ImageLoaded:\s+(.*)").Matches.Groups[1].Value.Trim()
    ...
    Write-Host ""
    Write-Host "[DLL NAO ASSINADA DETECTADA]" -ForegroundColor Red
    Write-Host "Horario: $($_.TimeCreated)"
    Write-Host ""

    Write-Host "DLL: $dll" -ForegroundColor Yellow
    Write-Host "Assinatura: INVALIDA / AUSENTE" -ForegroundColor Magenta
...
{
    Write-Host ""
    Write-Host "[POSSIVEL DLL HIJACKING DETECTADO]" -ForegroundColor Red
    Write-Host ""

    $SuspiciousDLLs | Sort-Object -Unique | ForEach-Object {

        Write-Host "DLL Suspeita: $_" -ForegroundColor Yellow
    }
}
else
{
    Write-Host ""
    Write-Host "[NENHUM HIJACKING DETECTADO]" -ForegroundColor Green
}
