...
Write-Host " Discord: https://discord.gg/qegq9MvR   " -ForegroundColor Magenta
Write-Host "========================================" -ForegroundColor Magenta
Write-Host ""
...
Write-Host "`n[*] RECYCLE BIN FORENSIC ANALYSIS" -ForegroundColor Cyan
    ...
    foreach ($user in $bamUsers) {
        ...
        foreach ($item in $bamProps) {
            ...
            if ($path.ToLower() -match "temp" -and $timeDiff.TotalMinutes -lt 30) {
                Write-Host "    [!!!] TEMP + LIVE EXECUTION (VERY SUSPICIOUS)" -ForegroundColor Red
            }

            # nome suspeito
            if ($path.ToLower() -match "loader|inject|cheat|spoofer|hack") {
                Write-Host "    [!!!] SUSPICIOUS EXECUTABLE NAME" -ForegroundColor Red
            }
        }
    }
...
Write-Host "`n[*] PREFETCH SUSPICIOUS PATTERNS" -ForegroundColor Cyan
...
foreach ($item in $pfList) {
    ...
    foreach ($p in $susPaths) {
        ...
        if ($item.Name -match $p) {
            ...
            Write-Host (" [!!!] EXECUTION FROM SUSPICIOUS PATH: {0}" -f $item.Name) -ForegroundColor Red
        }
    }

    if ($item.Runs -le 2 -and $item.Risk -eq "RECENT") {
        Write-Host (" [!!!] LOW EXEC COUNT + RECENT (Possible loader): {0}" -f $item.Name) -ForegroundColor Red
    }
}
...
if ($ResetBaseline -or -not (Test-Path $BaseFile)) {
    ...
    if ($base) {
        ...
        Write-Host "[+] Baseline pronto (FAST MODE)" -ForegroundColor Green
    }
...
Write-Host " FAST DLL CHECK" -ForegroundColor Cyan
Write-Host "======================`n" -ForegroundColor Cyan
...
Write-Host "`n[*] PROCESS / MEMORY / FILE SCANNER V4 (FORENSIC MODE)" -ForegroundColor Cyan
...
Write-Host "`n[*] FILE SYSTEM (RECENT ONLY)" -ForegroundColor Cyan
...
Write-Host "`n[*] SCAN COMPLETE (NO FALSE FLAG MODE)" -ForegroundColor Cyan


Write-Host "`n========================================" -ForegroundColor Magenta
Write-Host "        ADVANCED DNS CORRELATION        " -ForegroundColor Magenta
Write-Host "========================================" -ForegroundColor Magenta
...
Write-Host "`n[*] SYSMON DNS CORRELATION (Event ID 22)" -ForegroundColor Cyan
...
if ($sysmonEvents) {
    ...
    foreach ($evt in $sysmonEvents) {
        ...
        foreach ($d in $mediumRiskDomains) {
            if ($q -match $d) {
                ...
                Write-Host "`n [!] MEDIUM DNS (SYSMON)" -ForegroundColor Yellow
    ...
    Write-Host " Sysmon DNS logs not found." -ForegroundColor DarkGray
}
