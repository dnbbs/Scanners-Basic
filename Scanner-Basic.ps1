...
foreach ($Event in $SysmonEvents) {
    ...
    try {
        ...
        if (
            $LowerQuery -match '^\d{1,3}(\.\d{1,3}){3}$'
        ) {
...
try {
    ...
    if ($AnyDesk) {
        ...
        Write-Host "AnyDesk não está aberto. ID 10 será ignorado." -ForegroundColor Yellow
    }
}
catch {
...
Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "       [ID 5] PROCESS TERMINATED        " -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not $SysmonService -and -not $SysmonDriver) {

    Write-Host "[!] SYSMON NAO ENCONTRADO" -ForegroundColor Red
    Write-Host ""
    Write-Host "O Sysmon nao esta instalado neste computador." -ForegroundColor Yellow
    Write-Host ""

    return
}

Write-Host "[+] Sysmon encontrado." -ForegroundColor Green

if ($SysmonService) {
    Write-Host ("    Service : {0}" -f $SysmonService.Status) -ForegroundColor Gray
}

if ($SysmonDriver) {
    Write-Host ("    Driver  : {0}" -f $SysmonDriver.Status) -ForegroundColor Gray
}
...
if (-not $SysmonLog) {
    ...
    Write-Host ""
    Write-Host "[!] LOG DO SYSMON NAO ENCONTRADO" -ForegroundColor Red
    Write-Host ""
    Write-Host "O Sysmon foi encontrado, mas o log operacional nao existe." -ForegroundColor Yellow
    Write-Host ""

    return
}

if (-not $SysmonLog.IsEnabled) {

    Write-Host ""
    Write-Host "[!] LOG DO SYSMON ESTA DESABILITADO" -ForegroundColor Red
    Write-Host ""

    return
}

Write-Host "[+] Log Sysmon encontrado e habilitado." -ForegroundColor Green
Write-Host ""
...
while ($true) {
    ...
    $HoraInicio = Read-Host "Digite o horario de inicio (HH:mm:ss)"
    ...
    try {
        ...
        Write-Host ""
        Write-Host "[!] Horario de inicio invalido." -ForegroundColor Red
        Write-Host "    Use HH:mm:ss. Exemplo: 15:30:00" -ForegroundColor DarkGray
        Write-Host ""
        continue
    }

    if ($StartTime -lt $BootTime) {

        Write-Host ""
        Write-Host "[!] O horario de inicio ocorreu antes do boot." -ForegroundColor Red
        Write-Host ("    Boot   : {0}" -f $BootTime) -ForegroundColor Gray
        Write-Host ("    Inicio : {0}" -f $StartTime) -ForegroundColor Gray
        Write-Host ""
        continue
    }
    ...
    if ($StartTime -gt $Now) {
        ...
        Write-Host ""
        Write-Host "[!] O horario de inicio esta no futuro." -ForegroundColor Red
        Write-Host ""
        continue
    }

    $HoraFim = Read-Host "Digite o horario de fim (HH:mm:ss)"
    ...
    try {
        ...
        Write-Host ""
        Write-Host "[!] Horario de fim invalido." -ForegroundColor Red
        Write-Host "    Use HH:mm:ss. Exemplo: 16:30:00" -ForegroundColor DarkGray
        Write-Host ""
        continue
    }
    ...
    if ($EndTime -le $StartTime) {
        ...
        Write-Host ""
        Write-Host "[!] O horario final precisa ser maior que o inicial." -ForegroundColor Red
        Write-Host ""
        continue
    }
    ...
    if ($EndTime -gt $Now) {
        ...
        Write-Host ""
        Write-Host "[!] O horario de fim esta no futuro." -ForegroundColor Red
        Write-Host ""
        continue
    }
...
Write-Host ("Data             : {0}" -f $Today.ToString("yyyy-MM-dd")) -ForegroundColor White
Write-Host ("Boot             : {0}" -f $BootTime) -ForegroundColor Green
Write-Host ("Inicio da partida: {0}" -f $StartTime) -ForegroundColor Yellow
Write-Host ("Fim da partida   : {0}" -f $EndTime) -ForegroundColor Yellow
Write-Host ""
...
$Whitelist = @(
    ...
    "C:\Program Files (x86)\Microsoft\Edge\Application\151.0.4129.72\identity_helper.exe",
...
Write-Host "[*] Procurando Event ID 5..." -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    Write-Host ""
    Write-Host "[!] Erro ao consultar Event ID 5." -ForegroundColor Red
...
foreach ($Evt in $Events) {
    ...
    try {
        ...
        $NormalizedImage = $Image.Trim().Trim('"')

        if ($Whitelist -contains $NormalizedImage) {
            $Ignored++
            continue
        }
        ...
        Write-Host ("Time    : {0}" -f $Evt.TimeCreated) -ForegroundColor White
        Write-Host ("Process : {0}" -f $Image) -ForegroundColor White
        Write-Host ("PID     : {0}" -f $ProcessId) -ForegroundColor Gray

        if ($UtcTime) {
            Write-Host ("UtcTime : {0}" -f $UtcTime) -ForegroundColor DarkGray
        }
...
Write-Host ("Data       : {0}" -f $Today.ToString("yyyy-MM-dd")) -ForegroundColor White
Write-Host ("Inicio     : {0}" -f $StartTime.ToString("HH:mm:ss")) -ForegroundColor DarkGray
Write-Host ("Fim        : {0}" -f $EndTime.ToString("HH:mm:ss")) -ForegroundColor DarkGray
Write-Host ("Detectados : {0}" -f $Count) -ForegroundColor Yellow
Write-Host ("Whitelist  : {0}" -f $Ignored) -ForegroundColor Green
Write-Host ""

if ($Count -eq 0) {
    Write-Host "[+] Nenhum processo fora da whitelist foi encerrado nesse intervalo." -ForegroundColor Green
}
else {
    Write-Host "[!] Todos os processos fora da whitelist foram exibidos." -ForegroundColor Yellow
}
...
$WhitelistIPs = @(
    ...
    "2603:1063:2001:1106::365:ff1",
    "2603:1063:2001:1109::365:ff1",

    "2606:4700:7::a29f:80eb",
    "2606:4700:7::a29f:8aea",
    "2606:4700:3030::ac43:c351",
    ...
    "2804:14d:5cf9::c911:1ec0",
    "2804:14d:5cf9:1::c911:2518",
    "2804:14d:5cf9:1::c911:252b",
...
function Get-Classification {
    ...
    if ($WhitelistSet.Contains($RemoteIP)) {
        return "WHITELIST"
    }

    $RemoteType = Get-IPType $RemoteIP
    ...
    if ($RemotePort -in $CommonPorts) {
        return "NORMAL"
    }

    if ($RemotePort -eq 25565) {
        return "UNKNOWN"
    }
    ...
    if ($ProcessName -in $CommonProcesses) {
        return "UNKNOWN"
    }

    if ($ProcessName -match "powershell|pwsh|cmd|wscript|cscript") {
        return "REVIEW"
    }
...
Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "                 REMOTE CONNECTION SCANNER" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""

Write-Host "[*] Obtendo horario de inicializacao..." -ForegroundColor DarkGray
...
if ($null -ne $OS) {
    ...
    $BootTime = (Get-Date).AddDays(-1)
}

$ScanTime = Get-Date

Write-Host "[+] Boot : $($BootTime.ToString('yyyy-MM-dd HH:mm:ss'))" -ForegroundColor Green
Write-Host "[+] Scan : $($ScanTime.ToString('yyyy-MM-dd HH:mm:ss'))" -ForegroundColor Green
Write-Host ""

Write-Host "[*] Carregando processos..." -ForegroundColor DarkGray
...
foreach ($Process in $Processes) {
    ...
    try {
        $ProcessCache[[int]$Process.Id] = $Process.ProcessName
    }
    catch {
    }
}

Write-Host "[+] Processos carregados: $($ProcessCache.Count)" -ForegroundColor Green

Write-Host "[*] Carregando cache DNS..." -ForegroundColor DarkGray
...
try {
    ...
    foreach ($Entry in $DNSEntries) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($Entry.Name)) {
            ...
            if (-not $DNSCache.ContainsKey($Name)) {
                $DNSCache[$Name] = $Name
            }
        }
    }
...
Write-Host "[+] Cache DNS carregado." -ForegroundColor Green
Write-Host ""

Write-Host "[*] Verificando conexoes TCP atuais..." -ForegroundColor DarkGray

$CurrentConnections = Get-NetTCPConnection -ErrorAction SilentlyContinue

Write-Host "[+] Conexoes atuais verificadas: $($CurrentConnections.Count)" -ForegroundColor Green

$CurrentGroups = @{}

foreach ($Connection in $CurrentConnections) {

    $RemoteIP = Normalize-IP $Connection.RemoteAddress

    if ($RemoteIP -eq "-" -or
        $RemoteIP -eq "0.0.0.0" -or
        $RemoteIP -eq "::" -or
    ...
    $ProcessName = Get-ProcessNameFast $ProcessId

    $RemotePort = [int]$Connection.RemotePort

    $Key = "$ProcessName|$ProcessId|$RemoteIP|$RemotePort"
    ...
    if ($CurrentGroups.ContainsKey($Key)) {
        ...
        $CurrentGroups[$Key] = [PSCustomObject]@{
            Source       = "CURRENT"
            Process      = $ProcessName
            PID          = $ProcessId
            ...
            Protocol     = "TCP"
            IPVersion    = Get-IPVersion $RemoteIP
            RemoteType   = Get-IPType $RemoteIP
            PortName     = Get-PortName $RemotePort
            Count        = 1
            FirstSeen    = $ScanTime
            LastSeen     = $ScanTime
        }
...
Write-Host "[+] Destinos atuais unicos: $($CurrentGroups.Count)" -ForegroundColor Green
Write-Host ""

Write-Host "[*] Procurando Sysmon Event ID 3..." -ForegroundColor DarkGray
Write-Host "[*] Periodo: BOOT -> AGORA" -ForegroundColor DarkGray
...
if ($SysmonAvailable) {
    ...
    Write-Host "[+] Eventos Sysmon encontrados: $($SysmonEvents.Count)" -ForegroundColor Green
}
else {

    Write-Host "[!] Nenhum evento Sysmon Event ID 3 encontrado." -ForegroundColor Yellow
}
...
if ($SysmonAvailable) {
    ...
    foreach ($Event in $SysmonEvents) {
        ...
        try {
            ...
            $DestinationIP = Normalize-IP $Data["DestinationIp"]

            if ($DestinationIP -eq "-" -or
                $DestinationIP -eq "0.0.0.0" -or
                $DestinationIP -eq "::" -or
                $DestinationIP -eq "127.0.0.1" -or
                $DestinationIP -eq "::1") {

                continue
            }

            if ($WhitelistSet.Contains($DestinationIP)) {
                continue
            }
            ...
            if (-not [string]::IsNullOrWhiteSpace($Data["Image"])) {

                $ProcessName = Split-Path `
                    ([string]$Data["Image"]) `
                    -Leaf
            }
            ...
            $SourceIP = Normalize-IP $Data["SourceIp"]

            $SourcePort = 0

            [int]::TryParse(
                [string]$Data["SourcePort"],
                [ref]$SourcePort
            ) | Out-Null

            $Key = "$ProcessName|$ProcessId|$DestinationIP|$DestinationPort"

            if ($HistoricalGroups.ContainsKey($Key)) {

                $HistoricalGroups[$Key].Count++

                if ($Event.TimeCreated -lt $HistoricalGroups[$Key].FirstSeen) {
                    $HistoricalGroups[$Key].FirstSeen = $Event.TimeCreated
                }

                if ($Event.TimeCreated -gt $HistoricalGroups[$Key].LastSeen) {
                    $HistoricalGroups[$Key].LastSeen = $Event.TimeCreated
                }
            }
            else {

                $HistoricalGroups[$Key] = [PSCustomObject]@{
                    Source       = "SINCE_BOOT"
                    Process      = $ProcessName
                    PID          = $ProcessId
                    LocalIP      = $SourceIP
                    ...
                    Protocol     = "TCP"
                    IPVersion    = Get-IPVersion $DestinationIP
                    RemoteType   = Get-IPType $DestinationIP
                    PortName     = Get-PortName $DestinationPort
                    Count        = 1
...
Write-Host "[+] Conexoes historicas unicas: $($HistoricalGroups.Count)" -ForegroundColor Green
Write-Host ""

$Results = [System.Collections.Generic.List[object]]::new()

foreach ($Item in $CurrentGroups.Values) {
...
foreach ($Item in $HistoricalGroups.Values) {
...
$NormalCount = @(
    $Results |
    Where-Object { $_.Classification -eq "NORMAL" }
).Count
...
$WhitelistCount = @(
    $Results |
    Where-Object { $_.Classification -eq "WHITELIST" }
).Count

$Elapsed = $FinalTime - $GlobalStart
...
Write-Host "Boot:        $($BootTime.ToString('yyyy-MM-dd HH:mm:ss'))"
Write-Host "Execucao:    $($ScanTime.ToString('yyyy-MM-dd HH:mm:ss'))"
Write-Host "Final:       $($FinalTime.ToString('yyyy-MM-dd HH:mm:ss'))"
Write-Host "Tempo:       $($Elapsed.ToString('hh\:mm\:ss'))"
Write-Host "Sysmon:      $SysmonAvailable"
Write-Host ""

Write-Host "NORMAL    : $NormalCount" -ForegroundColor Green
Write-Host "UNKNOWN   : $UnknownCount" -ForegroundColor Yellow
Write-Host "REVIEW    : $ReviewCount" -ForegroundColor Red
Write-Host "WHITELIST : $WhitelistCount" -ForegroundColor DarkGray
Write-Host ""

if ($Results.Count -eq 0) {

    Write-Host "[OK] Nenhuma conexao encontrada." -ForegroundColor Green

}
else {

    Write-Host "============================================================" -ForegroundColor Cyan
    Write-Host "                    CONEXOES RELEVANTES" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
    Write-Host ""
...
Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "CURRENT    = conexao TCP existente no momento da execucao"
Write-Host "SINCE_BOOT = Sysmon Event ID 3 desde o boot ate a execucao"
Write-Host "WHITELIST  = IP remoto explicitamente ignorado"
Write-Host "NORMAL     = conexao comum / esperada"
Write-Host "UNKNOWN    = destino ou porta nao reconhecida"
Write-Host "REVIEW     = conexao que merece investigacao adicional"
Write-Host "============================================================" -ForegroundColor Cyan
